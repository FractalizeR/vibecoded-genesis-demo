...
// Добавляет вращательное движение вокруг оси Y + радиальный разлёт
//
vec3 spiralFlow(vec3 pos, float strength) {
  ...
  vec3 tangent;
  tangent.x = -sin(angle + spiralTwist) * angularVel;
  tangent.y = 0.0;
  tangent.z = cos(angle + spiralTwist) * angularVel;

  // Радиальная составляющая — отталкивание от центра
  // Сильнее у центра, слабее на периферии
  vec3 radial = vec3(0.0);
  if (dist3d > 0.01) {
    float radialStrength = strength * 0.5 / (dist3d + 0.5);
    radial = normalize(pos) * radialStrength;
  }
